...
class helper:
    def __init__(self):
        self.yaml = YAML()
        self.LoadSettings()
        self.port = urlparse(self.settings["config"]["api_url"]).port
        self.start_helper()
        self.method = [
    ...
    def start_helper(self):
        ...
        #     if proc.info["name"].contains("mahjong-helper"):
        #         return
        ...
        atexit.register(self.stop_helper)
...
"""
    ...
    def main(self, result):
        if result["method"] in self.method:
            ...
            requests.post(self.settings["config"]["api_url"], json=data, verify=False)
            if "liqi" in data.keys():  # 补发立直消息
                logger.success(f'[helper] 已发送：{data["liqi"]}')
                requests.post(
                    self.settings["config"]["api_url"], json=data["liqi"], verify=False
                )
